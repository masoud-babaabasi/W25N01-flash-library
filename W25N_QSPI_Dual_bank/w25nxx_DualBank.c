/*********************************************************************************************************
*
* File                : W25nxx_DualBank.c
* Hardware Environment: 
* Version             : V1.0
* By                  : M.Babaabsi 2022may
*
*
*********************************************************************************************************/
#include "w25nxx_DualBank.h"
...
uint8_t W25NXX_get_BUSSY(){
	...
	return ( (quad_bk1_sr & 0x01) | ( (quad_bk2_sr & 0x01) << 1) );
}
...
void W25NXX_ReadID(uint32_t * id1 , uint32_t * id2)
{
	...
	*id1 =(temp[0]<<16) | (temp[2]<<8) | temp[4];
	*id2 =(temp[1]<<16) | (temp[3]<<8) | temp[5];
}    
// read one page up to 4096 bytes
//	NumByteToRead <= 4096
...
{ 
	...
	uint32_t PageAdd = address >> 12;
	uint32_t tick_start;
	if( NumByteToRead == 0 ) return 0;
	if( NumByteToRead > W25N01_PageSize) NumByteToRead = W25N01_PageSize;
	if( NumByteToRead > W25N01_PageSize - (address & 0xfff ) ) NumByteToRead = W25N01_PageSize - (address & 0xfff);
...
//	cmd.AddressSize = QSPI_ADDRESS_16_BITS;
	...
	cmd.InstructionMode = QSPI_INSTRUCTION_1_LINE;
	cmd.Address = ( address / 2) &  0x7ff ; // column address = address & ( ( 1 << 11) - 1)
	cmd.AddressMode = QSPI_ADDRESS_1_LINE;
	cmd.AddressSize = QSPI_ADDRESS_16_BITS;
	cmd.DummyCycles = 8;
...
uint16_t W25NXX_Write_Page(uint8_t* pBuffer,uint32_t SectorWriteAddr,uint32_t NumByteToWrite)
{
	...
	if( NumByteToWrite > W25N01_PageSize - (address & 0xfff ) ) NumByteToWrite = W25N01_PageSize - (address & 0xfff );
...
//	cmd.AddressSize = QSPI_ADDRESS_16_BITS;
	...
	cmd.InstructionMode = QSPI_INSTRUCTION_1_LINE;
	cmd.Address = ( ( address / 2 ) & 0x7ff ); // column address
	cmd.AddressMode = QSPI_ADDRESS_1_LINE;
	cmd.AddressSize = QSPI_ADDRESS_16_BITS;
	cmd.DataMode = QSPI_DATA_4_LINES;
...
static uint8_t buff_repalce[ W25N01_EraseSize ] __attribute__((section(".GUI_NUMBYTES_section")));
...
uint8_t W25NXX_Erase_Block256K(uint32_t block_add)   
{   
	uint32_t tick_start;
  W25NXX_Write_Enable();	 
...
//	cmd.AddressSize = QSPI_ADDRESS_16_BITS;
	...
	buff[1] = (block_add >> 8)& 0xff;
	buff[2] = (block_add & 0xff);
	buff[3] = (block_add & 0xff);
...
uint8_t W25NXX_ChipErase(void){
	for(uint32_t i=0 ; i < W25N01_BlockNum * W25N01_BlockSize ; i += W25N01_BlockSize){
		if( W25NXX_Erase_Block256K( i ) == 0) return 0;
	}
	return 1;
}
//Waiting for free
